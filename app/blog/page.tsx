  ...
  return (
    <main style={{ minHeight: "100vh", background: "#0A0A0A", color: "#fff", fontFamily: "var(--font-dm-sans), sans-serif" }}>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "48px 24px" }}>
        <h1 style={{ fontSize: 32, fontWeight: 800, letterSpacing: "-0.03em", marginBottom: 8 }}>Blog</h1>
        <p style={{ fontSize: 15, color: "rgba(255,255,255,0.45)", marginBottom: 40, fontWeight: 300 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {posts.map((post: any) => (
              <Link key={post.slug} href={`/blog/${post.slug}`} style={{
                ...
                padding: "24px", textDecoration: "none", color: "#fff",
                transition: "border-color .2s",
              }}>
                {post.cluster && (
                  <span style={{
                    fontSize: 10, fontWeight: 600, letterSpacing: 2, textTransform: "uppercase",
                    color: C.teal, opacity: 0.7, marginBottom: 8, display: "block",
                  }}>{post.cluster}</span>
                )}

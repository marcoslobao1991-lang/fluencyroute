...
const frTrack = createTracker({ funnel: 'ingles', page: 'bridge', variant: 'A' })

// ═══════════════════════════════════════════════════════════════
// /bridge — Advertorial editorial light v4 (RESTAURADO 12/06 — a demo
// guiada vive em /bridge-demo; advertorial volta pra linha de frente)
  ...
  return (
    <aside style={{
      ...
      <a href={vslUrl} style={{
        ...
        onClick={() => {
          try { (window as unknown as { fbq?: (...a: unknown[]) => void }).fbq?.('trackCustom', 'BridgeCTAClick', { dest: 'vsl' }) } catch {}
          try { frTrack('cta_click', 'cta') } catch {}
        }}>
        Assistir a aula
...
export default function BridgePage() {
  ...
  useEffect(() => {
    ...
    // humano real (3s+ com aba visível) — mesma régua da demo, comparação justa
    let seenT: number | null = null
    let seenFired = false
    const armSeen = () => {
      if (seenFired) return
      if (document.visibilityState === 'visible' && seenT === null) {
        seenT = window.setTimeout(() => { seenFired = true; try { frTrack('gate_seen') } catch {} }, 3000)
      } else if (document.visibilityState !== 'visible' && seenT !== null) {
        clearTimeout(seenT)
        seenT = null
      }
    }
    armSeen()
    document.addEventListener('visibilitychange', armSeen)
    ...
    return () => {
      ...
      document.removeEventListener('visibilitychange', armSeen)
      if (seenT) clearTimeout(seenT)

# Briefing-mestre — Fluency Secrets

## 1. Produto

**Título de trabalho:** Fluency Secrets  
**Subtítulo editorial:** Como colocar o inglês no automático do seu cérebro e nunca mais traduzir mentalmente
**Formato:** livro digital pago, experiência web premium e aulas gravadas por capítulo  
**Rota isolada:** `app.fluencyroute.com.br/fluency-secrets`

O produto não é uma isca digital, uma apostila ou um curso transformado em PDF. É um livro curto, denso e terminável, capaz de provocar uma mudança de paradigma e produzir uma primeira prova pessoal do mecanismo.

## 2. Função no funil

O ebook ensina **o que está acontecendo** e **por que o leitor continua travado**. A oferta principal, definida depois, entregará **o como pronto**: sequência, materiais, progressão, acompanhamento e execução.

Fluxo desejado:

1. Compra do livro.
2. Leitura e aplicação do experimento.
3. Mudança de diagnóstico sobre a própria trava.
4. Experimento prático e primeira evidência de automatização.
5. Encerramento editorial completo, sem ponte comercial.
6. Qualquer webinar ou oferta acontece fora do livro, em outro momento e por outro contexto.

O livro precisa ser valioso mesmo que o leitor nunca compre a oferta seguinte.

## 3. Big Domino

> A fluência não acontece quando você aprende mais inglês. Ela acontece quando o inglês que você já conhece é treinado até funcionar automaticamente, sem depender da tradução mental.

Consequência lógica:

> Eu não preciso medir progresso pela quantidade de conteúdo. Preciso transformar inglês essencial em habilidade disponível.

## 4. Inimigo e novo diagnóstico

**Dor visível:** traduzir mentalmente.  
**Causa estrutural:** tratar uma habilidade como matéria.  
**Comportamento que mantém a trava:** consumo aleatório de conteúdo novo sem domínio do anterior.  
**Novo diagnóstico:** o leitor provavelmente conhece mais inglês do que consegue acessar em tempo real.

O leitor não é incapaz, velho demais ou sem talento. Ele recebeu um modelo de treinamento inadequado para a habilidade que deseja desenvolver.

## 5. Frame persuasivo

Estrutura híbrida:

- Epiphany Bridge.
- Big Domino.
- Quebra das três falsas crenças: veículo, interna e externa.
- Escada de crenças.
- Prova pessoal.
- Nova identidade.
- Autonomia para avaliar qualquer treino pelo mecanismo, não pela quantidade prometida.

Escada de crenças:

1. Eu não preciso aprender tudo.
2. Preciso dominar o essencial.
3. Saber não significa conseguir usar.
4. Inglês é habilidade, não matéria.
5. Habilidades são automatizadas com treino.
6. Tradução mental é uma etapa intermediária lenta.
7. Meu ouvido e minha fala podem ser treinados.
8. Trechos concentrados aceleram a automatização.
9. Consigo perceber essa mudança em mim.
10. Agora consigo distinguir conteúdo entregue de habilidade construída.

## 6. História de origem

Marcos trabalhava como vendedor e passava muitas horas dirigindo. Na época, usava um pendrive no som do carro. Havia poucos arquivos nele, entre eles conteúdos de vendas em inglês de Jordan Belfort e Grant Cardone. Marcos já queria trabalhar vendendo em dólar.

Por falta de variedade, escutava os mesmos áudios em loop durante horas de estrada. Sem plano ou método, percebeu que trechos antes rápidos e embolados começavam a ficar claros e automáticos.

Depois, aplicou o mesmo princípio à pronúncia: repetiu discursos curtos junto com os falantes, usando shadowing, até decorar alguns deles. Como esses discursos continham palavras, sons e blocos recorrentes do inglês essencial, a habilidade começou a se transferir para outros contextos.

Nome interno da descoberta: **O Acidente do Pendrive**.

Frase central:

> Eu não melhorei porque estava aprendendo coisas novas. Melhorei porque parei de abandonar as mesmas coisas antes que elas se tornassem automáticas.

## 7. Política editorial contra “pitch disfarçado”

O corpo do livro não apresentará:

- História de fundação da Fluency Route.
- Quantidade de alunos.
- Prints ou parede de depoimentos.
- Módulos, bônus, preço ou garantia.
- Escassez ou urgência.
- Capítulo de apresentação comercial.
- Transição “agora que você entendeu, conheça o produto”.

A autoridade virá da experiência real, da clareza do raciocínio, das referências corretas e da capacidade de o leitor testar o mecanismo.

A Fluency Route aparece somente como identidade discreta da publicação e da plataforma. O livro não anuncia webinar, call, curso ou próxima etapa.

## 8. Voz

- Primeira pessoa, direta e íntima.
- Inteligente sem academicismo.
- Confiante sem autopromoção.
- Concreta, visual e baseada em cenas.
- Frases curtas nos momentos de descoberta.
- Explicações mais respiradas nos conceitos densos.
- Sem promessas milagrosas.
- Sem tratar o leitor como preguiçoso.
- Sem “internetês” excessivo no manuscrito.

O texto precisa soar como um bom livro de negócios/comportamento, não como uma transcrição de VSL.

## 9. Estrutura do manuscrito

### Prólogo — O pendrive quase vazio

A estrada, a ambição de vender em dólar, a repetição involuntária e o momento em que os áudios parecem desacelerar sem que o arquivo mude.

### Parte I — O diagnóstico que muda tudo

1. A conversa que chega atrasada
2. Entender não é conseguir
3. Aprender inglês não é treinar inglês
4. O vício de sempre recomeçar

### Parte II — Como o inglês entra no automático

5. O áudio não ficou mais lento
6. Você não conversa com palavras
7. Fluência Essencial

### Parte III — A vantagem sem graça

8. Um minuto dominado
9. As duas repetições
10. A linha do automático
11. Conversar e sustentar uma ideia

### Parte IV — A prova

12. O Experimento Fluency Secrets
13. O próximo trecho não começa do zero

### Parte V — A expansão

14. Do primeiro episódio à imersão

### Epílogo — O inglês que fica

Fechamento da história, síntese do novo diagnóstico e instrução final autossuficiente.

### Notas e fontes

Separação clara entre experiência pessoal, modelo editorial e evidência científica.

## 10. Frame de capítulo

1. Cold open.
2. Crença antiga.
3. Quebra de padrão.
4. História ou analogia.
5. Novo princípio.
6. Diagrama explicativo.
7. Caixa “Fluency Secret”.
8. Experimento ou pergunta de observação.
9. Mudança de crença e ponte para o próximo capítulo.

## 11. Sistema visual

Direção: livro premium de negócios e comportamento. Fundo marfim, tinta quase preta, teal da marca como cor funcional e amarelo queimado para destaques editoriais. Tipografia de leitura serifada; interface em sans-serif.

Mecanismos e diagramas editoriais:

- O Imposto da Segunda Conversa.
- O Inglês que Some na Hora H.
- O Vício de Sempre Recomeçar.
- O Loop de Automatização.
- O Caminho Direto para o significado.
- As duas versões de uma conversa.
- Conhecimento consciente versus execução automática.
- Consumo Aleatório versus Treino Concentrado.
- Repetição Continuada versus Espaçada.
- O Núcleo que Destrava o Resto.
- O Filtro do Essencial: recorrência, combinação e relevância.
- O Inglês Puxa Mais Inglês: trecho → episódio → discurso → imersão.

## 12. Experiência do leitor

- Mobile-first.
- Índice retrátil.
- Progresso salvo localmente.
- Retomada do último capítulo.
- Ajuste de fonte.
- Modo claro/escuro.
- Marcação de capítulo concluído.
- Diagramas responsivos e acessíveis.
- Navegação anterior/próximo.
- Aulas por capítulo podem existir na área de membros, mas não interrompem nem comercializam dentro da leitura.
- Nenhum CTA comercial dentro do ebook.

## 13. Critério de sucesso

Ao terminar, o leitor deve conseguir dizer:

> Eu achava que precisava aprender mais inglês. Agora entendo que preciso automatizar o inglês essencial que já encontrei — e consigo perceber o início dessa mudança no meu próprio ouvido.
